...
from smtp_client.exceptions import InvalidTemplate
...
class SmtpTemplate:
    def __init__(
        ...
        **kwargs,
        ...
        if template:
            try:
                template_obj: EmailTemplates = EmailTemplates.objects.get(
                ...
                logger.critical(
                    f"The template '{template}' is no longer valid, please check "
                    "the configuration"
                )
                raise InvalidTemplate(e, template_name=template)

        if subject is None and body is None:
            raise ValueError(
                "If the template name is not provided then the subject and body "
                "fields are required"
            )
    ...
    def send(self, **kwargs) -> None:
        ...
        s = Smtp()
        msg = s.mime_build(
            html=self.env_body.render(context),
