...
function delete_phone(id) {
    ...
    $.ajax({
        ...
        .fail(function (jqXHR, status, error) {
            alert_error("Failed to delete address ", error);
        });
}
    ...
    $.ajax({
        ...
        .done(function (res) {
            ...
            if (res["primary"]) {
                ...
                $("input[name=primary]", "#phones-" + res["id"]).prop(
                    "checked",
                    true
                );
            }
    ...
    $.ajax({
        ...
        .done(function (res) {
            ...
            if (res["primary"]) {
                ...
                $("input[name=primary]", "#addresses-" + res["id"]).prop(
                    "checked",
                    true
                );
            }
...
function update_software(e) {
    ...
    $.ajax({
        ...
        .done(function (r) {
            if (method === "POST") {
                software[r["id"]] = r;
                software[r["id"]]["software_name"] = $(
                    "option[value='id_" + r["software"] + "']",
                    f
                ).text();
                $("#software-list").append(
                    softwareListTemplate(software[r["id"]])
                );
            } else {
        ...
        .fail(function (jqXHR, status, error) {
            ...
            Object.keys(jqXHR.responseJSON).forEach(function (k) {
                console.log(k + ": " + jqXHR.responseJSON[k]);
                if (k === "non_field_errors") {
                    $(f).prepend(
                        '<div class="invalid-feedback">' +
                            jqXHR.responseJSON[k] +
                            "</div>"
                    );
                    error_msg += "<br />" + jqXHR.responseJSON[k];
                } else if (k === "software") {
                    $(f)
                        .find("[name=" + k + "]")
                        .addClass("is-invalid")
...
function delete_software(e) {
    ...
    $.ajax({
        ...
        .fail(function (jqXHR, status, error) {
            alert_error("Failed to delete software ", jqXHR);
        });
}
...
function employee_undo() {
    function c(t, e) {
        if (t.val() !== employee_import[e]) {
            if (employee_import[e].length === 0) {
                return false;
            }
            if (employee_import[e] === "None") {
                return false;
            }
            return true;
        } else {
...
function populate_select(field, url) {
    ...
    $.ajax({
        ...
        .done(function (res) {
            if (res.length > 0) {
                res.forEach(function (e, i) {
                    if (s.indexOf(e["id"]) === -1) {
                        select.append(selectTemplate(e));
                    }
                });
            }
        })
        .fail(function (jqXHR, status, error) {
            alert_error(
                "There was an error loading the page, please refresh." + error
            );
        });
    $(select).selectpicker({
        style: "",
